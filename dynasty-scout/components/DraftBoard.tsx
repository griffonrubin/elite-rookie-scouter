...
function DraftBoardContent({ players }: DraftBoardProps) {
    ...
    const [dataset, setDataset] = useState<BoardDataset>(
        (['snapshot', 'rankings', 'traits', 'production', 'seasons', 'advanced', 'scouting'].includes(searchParams.get('view') || '')
    ...
    const filteredPlayers = useMemo(() => {
        ...
        return [...result].sort((a, b) => {
            ...
            switch (sortKey) {
                ...
                case 'flock':  va = (a as any).flock_sf_rank ?? MISS; vb = (b as any).flock_sf_rank ?? MISS; break;
                case 'pfn':    va = (a as any).pfn_rank ?? MISS; vb = (b as any).pfn_rank ?? MISS; break;
                case 'tank':   va = (a as any).tank_rank ?? MISS; vb = (b as any).tank_rank ?? MISS; break;
                case 'tdn':    va = (a as any).tdn_rank ?? MISS; vb = (b as any).tdn_rank ?? MISS; break;
                case 'brug':   va = (a as any).brugler_rank ?? MISS; vb = (b as any).brugler_rank ?? MISS; break;
                case 'dj':     va = (a as any).dj_rank ?? MISS; vb = (b as any).dj_rank ?? MISS; break;
                ...
                case 'career_td': {
                    const getTd = (p: any) => p.position === 'QB' ? (p.career_pass_tds ?? 0) : ((p.career_rush_tds ?? 0) + (p.career_rec_tds ?? 0));
                    va = getTd(a) || MISS; vb = getTd(b) || MISS; break;
                }
                case 'career_scr': {
                    const getScr = (p: any) => p.position === 'QB' ? (p.career_pass_yards ?? 0) : (p.career_scrim_yards ?? 0);
                    va = getScr(a) || MISS; vb = getScr(b) || MISS; break;
                }
                case 'wr_yprr':   va = (a as any).wr_yprr ?? MISS; vb = (b as any).wr_yprr ?? MISS; break;
                case 'adot':      va = (a as any).wr_adot ?? MISS; vb = (b as any).wr_adot ?? MISS; break;
                case 'wr_drop':   va = (a as any).wr_drop_rate ?? MISS; vb = (b as any).wr_drop_rate ?? MISS; break;
                case 'contested': va = (a as any).wr_contested ?? MISS; vb = (b as any).wr_contested ?? MISS; break;
                case 'yac_rec':   va = (a as any).wr_yac_per_rec ?? MISS; vb = (b as any).wr_yac_per_rec ?? MISS; break;
                case 'slot':      va = (a as any).wr_slot_rate ?? MISS; vb = (b as any).wr_slot_rate ?? MISS; break;
                case 'rb_yac':    va = (a as any).rb_yds_after_contact ?? MISS; vb = (b as any).rb_yds_after_contact ?? MISS; break;
                case 'mtf':       va = (a as any).rb_mtf_pct ?? MISS; vb = (b as any).rb_mtf_pct ?? MISS; break;
                case 'rb_yprr':   va = (a as any).rb_yprr ?? MISS; vb = (b as any).rb_yprr ?? MISS; break;
                    ...
                    <div className="flex items-center gap-1.5">
                        ...
                        <Select value={sortKey} onValueChange={(v: SortKey) => { setSortKey(v); setSortDir(DEFAULT_DESC.includes(v as SortKey) ? 'desc' : 'asc'); }}>
                            ...
                            <SelectContent>
                                ...
                                <SelectItem value="flock">Flock Fantasy SF</SelectItem>
                    ...
                    <div className="flex items-stretch px-4 py-0 border-b border-white/[0.06] gap-0 min-h-[46px] sticky top-[54px] z-20"
                        ...
                        >
                            {colDefs.map((col, i) => (
                                ...
                                <TooltipTrigger asChild>
                                <div
                                    className={`flex items-center justify-center text-center min-h-[40px] ${i === 0 ? 'border-l border-border/30' : ''} ${col.key === 'fp' || col.key === 'pfn' || col.key === 'flock_sf' || col.key === 'avg_rank' || col.key === 'tier' || col.key === 'age' || col.key === 'career_td' || col.key === 's1' || col.key === 'hist_comp2' ? 'border-l border-border/20' : ''}`}
                                >
                                    {col.sortKey ? (
                                        <button
