    ...
    return (
        <div className="rounded-xl border border-border/40 bg-card/40 overflow-hidden">
            <div className="px-5 py-4 border-b border-border/30 bg-muted/10 flex items-center justify-between">
                <span className="text-xs font-bold uppercase tracking-widest text-muted-foreground/70">Production Share</span>
                <span className="text-[10px] text-muted-foreground/40 font-mono">by season</span>
            </div>
            ...
            <div className="p-5 space-y-5">
                {data.map(row => {
                    ...
                    return (
                        <div key={row.season} className="space-y-2">
                            {/* Season header */}
                            <div className="flex items-center gap-2">
                                <span className="text-xs font-bold font-mono text-muted-foreground w-10">{row.season}</span>
                                <span className="text-[11px] text-muted-foreground/50 truncate">{row.school}</span>
                            </div>

                            {/* Dominator bar */}
                            {hasDom && (
                                <div className="grid grid-cols-[60px_1fr_52px] items-center gap-3">
                                    <span className="text-[9px] text-muted-foreground/40 uppercase tracking-wide text-right">DOM%</span>
                                    <div className="relative h-3 bg-border/20 rounded-full overflow-hidden">
                                        ...
                                        <div className="absolute top-0 h-full w-px bg-emerald-400/30" style={{ left: `${(ELITE_DOM / maxDom) * 100}%` }} />
                                        <div className="absolute top-0 h-full w-px bg-white/8"      style={{ left: `${(GOOD_DOM / maxDom) * 100}%` }} />
                                    </div>
                            ...
                            {hasMkt && (
                                <div className="grid grid-cols-[60px_1fr_52px] items-center gap-3">
                                    <span className="text-[9px] text-muted-foreground/40 uppercase tracking-wide text-right">MKT%</span>
                                    <div className="relative h-3 bg-border/20 rounded-full overflow-hidden">
                                        {mkt != null && (
                                            <div
            ...
            <div className="px-5 py-3 border-t border-border/20 flex flex-wrap gap-4 text-[9px] text-muted-foreground/40">
                <span><span className="text-emerald-400 font-bold">DOM ≥25%</span> Elite</span>
                <span><span className="text-cyan-400 font-bold">≥15%</span> Starter</span>
                <span><span className="text-violet-400 font-bold">MKT</span> Pass yards share</span>
            </div>
        </div>
    );
}
