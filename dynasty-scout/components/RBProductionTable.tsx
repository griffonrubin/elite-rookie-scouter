...
export function RBProductionTable({ stats, peerAdvanced, playerId, rbCareer, peerRBAdv }: Props) {
    if (!stats.length && !rbCareer) return null;
    if (peerAdvanced.length < 3 && (!peerRBAdv || peerRBAdv.length < 3)) return null;
    ...
    const carFumbles = fumbleSeasons.length > 0
        ...
        : null;
    const carYacCont   = rbCareer?.yds_after_contact != null ? rbCareer.yds_after_contact
        : (Math.round(stats.reduce((s, r) => s + (r.yards_after_contact ?? 0), 0)) || null);
    const carExplosive = rbCareer?.explosive_attempts != null ? rbCareer.explosive_attempts
        : (Math.round(stats.reduce((s, r) => s + (r.explosive_run_rate ?? 0) * (r.rush_attempts ?? 0), 0)) || null);
    const carBreakaway = rbCareer?.breakaway_attempts != null ? rbCareer.breakaway_attempts
        : (Math.round(stats.reduce((s, r) => s + (r.breakaway_run_rate ?? 0) * (r.rush_attempts ?? 0), 0)) || null);
    const carMtf       = rbCareer?.avoided_tackles != null ? rbCareer.avoided_tackles
        : (stats.reduce((s, r) => s + (r.missed_tackles_forced ?? 0), 0) || null);
    const carRoutes    = rbCareer?.routes_run != null ? rbCareer.routes_run
        : (stats.reduce((s, r) => s + (r.routes_run ?? 0), 0) || null);
    const carYpc = carRushAtt > 0 ? carRushYds / carRushAtt : null;
    const useCareerTable = peerRBAdv && peerRBAdv.length >= 3;
    const peers = useCareerTable ? peerRBAdv! : null;
    ...
    const peerFumbles   = peerAdvanced.filter(p => p.fumbles != null).map(p => n(p.fumbles));
    const peerRecYds    = peerAdvanced.map(p => n(p.rec_yards));
    const peerYacCont   = useCareerTable ? peers!.map(p => n(p.yds_after_contact)).filter(v => v > 0) : peerAdvanced.map(p => n(p.yac_contact));
    const peerExplosive = useCareerTable ? peers!.map(p => n(p.explosive_attempts)).filter(v => v > 0) : peerAdvanced.map(p => n(p.explosive_att));
    const peerBreakaway = useCareerTable ? peers!.map(p => n(p.breakaway_attempts)).filter(v => v > 0) : peerAdvanced.map(p => n(p.breakaway_att));
    const peerMtf       = useCareerTable ? peers!.map(p => n(p.avoided_tackles)).filter(v => v > 0)   : peerAdvanced.map(p => n(p.mtf));
    const peerRoutes    = useCareerTable ? peers!.map(p => n(p.routes_run)).filter(v => v > 0)         : peerAdvanced.map(p => n(p.routes));
    type MetricSpec = { label: string; abbr: string; value: number | null; arr: number[]; fmt: (v: number) => string; higherBetter?: boolean; };
    const metrics: MetricSpec[] = [
        ...
        { label: 'Yds After Cont.', abbr: 'YAC',      value: carYacCont,                         arr: peerYacCont,   fmt: v => v.toLocaleString() },
        { label: 'Explosive (10+)', abbr: 'EXPL',     value: carExplosive,                       arr: peerExplosive, fmt: v => String(v) },
        { label: 'Breakaway (15+)', abbr: 'BKWY',     value: carBreakaway,                       arr: peerBreakaway, fmt: v => String(v) },
        { label: 'Avoided Tackles', abbr: 'MTF',      value: carMtf,                             arr: peerMtf,       fmt: v => String(v) },
        { label: 'Routes Run',      abbr: 'ROUTES',   value: carRoutes,                          arr: peerRoutes,    fmt: v => String(v) },
                ...
                {metrics.map(({ label, abbr, value, arr, fmt, higherBetter = true }) => {
                    ...
                    return (
                        <div
                            ...
                            title={label + ': ' + fmt(value) + ' · ' + pct + 'th pct in class'}
                        >
                            <div className={cn('text-sm font-black font-mono leading-none', text)}>{fmt(value)}</div>
