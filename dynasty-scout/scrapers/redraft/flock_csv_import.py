...
Deliberately IGNORED: the Sleeper, ESPN and CBS columns. Those three are
already scraped live and deeper; importing them too would enter the same
opinion into the consensus twice and silently double its weight.
...
class FlockCsvImport(BaseRedraftScraper):
    ...
    def fetch(self):
        ...
        missing = [c for c in COLUMN_SOURCES if c not in rows[0]]
        if missing:
            raise ValueError(
        ...
        collected = {col: [] for col in COLUMN_SOURCES}
        unmatched_names = set()
        ...
        for row in rows:
            ...
            if not pid:
                ...
                if any(to_rank(row.get(c)) for c in COLUMN_SOURCES):
...
if __name__ == "__main__":
    ...
    args = ap.parse_args()
    sys.exit(FlockCsvImport(args.file).run())
