...
export function getColDefs(pos: string): ColDef[] {
    ...
    return [FORTY, RAS, ARM, HAND, STARS, FP, KTC, FC, DN, TIER]; // 10 cols
}

/** CSS grid-template-columns string for the right dynamic section. */
export function getGridTemplate(pos: string): string {
    if (pos === 'ALL') {
        // 10 cols — measurables tight on left, ranking cols right
        return '0.65fr 0.65fr 0.65fr 0.65fr 0.65fr 0.8fr 0.8fr 0.8fr 0.8fr 1fr';
    }
    if (pos === 'QB') {
        // 9 cols
        return '0.65fr 0.65fr 0.65fr 1fr 0.85fr 0.7fr 0.8fr 0.8fr 1fr';
    }
    // RB / WR / TE — 8 cols
    return '0.65fr 0.65fr 0.65fr 0.9fr 1fr 0.8fr 0.8fr 1fr';
}
